<template>
  <div class="h-screen flex items-center justify-center">
    <div class="text-center">
      <CIcon name="i-heroicons-arrow-path" class="animate-spin mx-auto mb-4" size="2xl" />
...
onMounted(async () => {
  try {
    ...
    const response = await $fetch<{ success: boolean }>(`/api/wiki/${welcomePath}`)
    
    if (response.success) {
      navigateTo(`/public/${userId}/welcome.md`)
    } else {
      // If no welcome file, redirect to public wiki root to show file listing
