...
export default defineEventHandler(async (event) => {
  ...
  const directoryPath = decodedPath.endsWith('/') ? decodedPath : `${decodedPath}/`
  ...
  try {
    ...
    console.log(`Found ${objectsToDelete.length} objects to delete in directory`)
    ...
    for (let i = 0; i < objectsToDelete.length; i += batchSize) {
      ...
      if (deleteResponse.Errors && deleteResponse.Errors.length > 0) {
        ...
        throw new Error(`Failed to delete some objects: ${deleteResponse.Errors[0].Message}`)
      }
    ...
    throw createError({
      statusCode: 500,
      statusMessage: `Failed to delete directory: ${error.message || 'Unknown error'}`
    })
  }
})